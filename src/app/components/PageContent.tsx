...
import { AlertCircle, Github, Moon, Plus, Sun, Trash2, Upload } from "lucide-react";
...
export function PageContent() {
  ...
  useEffect(() => {
    const fetchStarCount = async () => {
      try {
        const response = await fetch("https://api.github.com/repos/creeponsky/which-choice");
  ...
  const handleFiles = async (files: File[]) => {
    ...
    if (hasCompression) {
      setCompressionWarning(
        `Some images were larger than ${MAX_IMAGE_DIMENSION}px and have been automatically resized for better performance while maintaining aspect ratio.`
      );
    }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className={`flex items-center gap-2 ${
                  theme === "dark"
                    ? "border-zinc-700 hover:bg-zinc-800 hover:text-zinc-200"
                    : "border-zinc-200 hover:bg-zinc-100 hover:text-zinc-900"
                }`}
                onClick={() => window.open("https://github.com/creeponsky/which-choice", "_blank")}>
                <Github className="h-4 w-4" />
                <span className="hidden sm:inline">GitHub</span>
                {starCount !== null && (
                  <span
                    className={`px-1.5 py-0.5 rounded-full text-xs font-medium
                    ${theme === "dark" ? "bg-zinc-700 text-zinc-200" : "bg-zinc-100 text-zinc-700"}`}>
                    {starCount}
                  </span>
                )}
              </Button>
      ...
      {images.length === 0 ? (
        <Card
          className={`h-[300px] flex flex-col items-center justify-center border-2 border-dashed relative transition-colors ${
            theme === "dark"
          ...
          <p className={theme === "dark" ? "text-zinc-500" : "text-zinc-400"}>or click to select files</p>
        </Card>
      ) : (
